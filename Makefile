...
.PHONY: test-e2e-router-plugins
test-e2e-router-plugins: ## Run router scheduler plugin e2e tests.
	@command -v kind >/dev/null 2>&1 || { echo "Kind is not installed."; exit 1; }
	@TEST_CATEGORY=router-plugins ./test/e2e/setup.sh
	@KUBECONFIG=/tmp/kubeconfig-e2e go test -v -timeout=15m ./test/e2e/router-plugins/...
