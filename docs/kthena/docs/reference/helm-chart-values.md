...
| networking.kthenaRouter.priorityQueue.enabled | bool | `false` | Enable the request priority queue (default strategy: user fairness). |
| networking.kthenaRouter.priorityQueue.fairness.inputTokenWeight | float | `1` | User-fairness strategy: weight multiplier for input tokens. |
| networking.kthenaRouter.priorityQueue.fairness.outputTokenWeight | float | `2` | User-fairness strategy: weight multiplier for output tokens. |
| networking.kthenaRouter.priorityQueue.fairness.windowSize | string | `"1h"` | User-fairness strategy: sliding window duration for token usage tracking. |
| networking.kthenaRouter.priorityQueue.maxConcurrent | int | `0` | Queue-level global total inflight request limit admitted to backends.<br/> Shared by all strategies; size it as estimated per-pod concurrency x pod count.<br/> `0` or unset uses the router default (16). |
| networking.kthenaRouter.priorityQueue.sessionBoost.enabled | bool | `false` | Switch the priority queue to the session-boost strategy.<br/> Replaces the default per-user fairness strategy with session-aware boosting (the two strategies are mutually exclusive). |
| networking.kthenaRouter.priorityQueue.sessionBoost.gracePeriod | string | `"0s"` | Wait time after a request completes for a same-session follow-up.<br/> Disabled by default (`0s`). |
| networking.kthenaRouter.priorityQueue.sessionBoost.header | string | `"X-Session-ID"` | HTTP header used to identify conversation sessions. |
| networking.kthenaRouter.priorityQueue.sessionBoost.maxSessions | int | `4096` | Maximum number of recently-completed sessions kept warm for boosting.<br/> Bounds an LRU cache; the least-recently-used session is evicted automatically.<br/> Size it by the number of concurrent conversations to keep boosted. |
| networking.kthenaRouter.priorityQueue.sessionBoost.pollInterval | string | `"100ms"` | Interval at which the queue polls backend pod metrics for capacity. |
